...
# 📥 Direct Downloader Function (Updated with newer API implementation)
async def download_via_shruti(link: str, is_video: bool = False):
    ...
    try:
        ...
        return None
# New download_song function from second file
    ...
    try:
        ...
        if os.path.exists(file_path):
            try:
                ...
                pass
        return None
# New download_video function from second file
...
class YouTubeAPI:
    ...
    # New video method from second file
    ...
    # ─────────────────────────────────────────────
    # 🔥 MODIFIED DOWNLOAD (USES NEW SHRUTI API DIRECTLY WITH FALLBACK)
    # ─────────────────────────────────────────────
    async def download(
        self,
        link: str,
        ...
        if videoid: link = self.base + link
        # 1️⃣ First try: New Shruti API Download Attempt
        is_video_req = True if video or songvideo else False
        
        try:
            # Try the newer API implementation first
            ...
            print(f"New API Failed, trying fallback: {e}")
        # 2️⃣ Second try: Legacy Shruti API (via download_via_shruti)
        try:
            downloaded_file = await download_via_shruti(link, is_video=is_video_req)
            if downloaded_file:
                return downloaded_file, True  # Success
        except Exception as e:
            print(f"Legacy API Failed, trying yt-dlp fallback: {e}")
        # 3️⃣ Final Fallback (YT-DLP) - If both API methods fail
